...
//安全公告控制器
class SafeInfoTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "SafeInfoCell", for: indexPath) as! DisplayTableViewCell
            ...
            if let mimeType = httpResponse.mimeType, mimeType == "text/html",
                        ...
                        let string = String(data: data, encoding: .utf8) {
                            DispatchQueue.main.async {
                                ...
                                let lines = str.replacingOccurrences(of: "\t", with: "").split(separator: "\r\n")
                                for i in lines{
                                    let symbol = i.split(separator: ">")
                                    //print(symbol[0])
    ...
    */
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        // Get the new view controller using segue.destination.
        // Pass the selected object to the new view controller.
        let SafetyViewController = segue.destination as! PageViewController
